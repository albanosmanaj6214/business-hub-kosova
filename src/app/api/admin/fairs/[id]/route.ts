...
// PATCH allows admins to edit the targetSectors[] of a fair and optionally
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  const fair = await prisma.tradeFair.update({
    where: { id: params.id },
    data: {
      targetSectors,
  ...
  if (body.notifySector === true && targetSectors.length > 0) {
    ...
    if (users.length) {
      const title = `Event i ri për sektorin: ${sectorsLabel(targetSectors)}`
      const message = (fair.nameSq || fair.name).slice(0, 240)
